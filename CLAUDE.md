...
- **Entry point**: `App.js` → wraps everything in `SystemProvider` → renders `AppNavigator`
- **Navigation**: `src/navigation/AppNavigator.js` — native stack navigator with screens: Splash → Auth → Home, and secondary screens (Profile, Emergency, Support, About, ChangePassword)
...
Auth flow: Splash checks existing session via `onAuthStateChanged` → Auth screen for login/register → Home.
...
- `ThemeContext.js` in `src/context/` — exists but is not wired into `App.js` or any screen
...
- `AuthScreen` — `signInWithEmailAndPassword`, `createUserWithEmailAndPassword`, `sendPasswordResetEmail`
- `SplashScreen` — `onAuthStateChanged` routes returning users directly to Home, bypassing Auth
